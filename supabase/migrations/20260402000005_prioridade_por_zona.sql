...
ADD COLUMN IF NOT EXISTS prioridade_zona_habilitada boolean NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS prioridade_zona_bonus_dentro integer NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS prioridade_zona_bonus_fora integer NOT NULL DEFAULT 0;

ALTER TABLE public.criancas
ADD COLUMN IF NOT EXISTS zona_atendimento_id uuid NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tc.constraint_schema = 'public'
      ...
      AND tc.constraint_name = 'criancas_zona_atendimento_id_fkey'
  ) THEN
    ALTER TABLE public.criancas
    ADD CONSTRAINT criancas_zona_atendimento_id_fkey
    FOREIGN KEY (zona_atendimento_id) REFERENCES public.zonas_atendimento(id)
    ON DELETE SET NULL;
  END IF;
END $$;
...
BEGIN
  ...
  ordered_children AS (
    SELECT 
      ...
      c.programas_sociais,
      c.prioridade,
      COALESCE(ps.peso_total, 0) + COALESCE(pr.peso_remanejamento, 0) + COALESCE(bz.bonus_cmei1, 0) AS peso_total_cmei1,
      COALESCE(ps.peso_total, 0) + COALESCE(pr.peso_remanejamento, 0) + COALESCE(bz.bonus_cmei2, 0) AS peso_total_cmei2,
      ROW_NUMBER() OVER (
