...
AS $$
  ...
  LEFT JOIN public.cmeis cmei1 ON c.cmei1_preferencia = cmei1.id
  LEFT JOIN public.cmeis cmei2 ON c.cmei2_preferencia = cmei2.id
  LEFT JOIN public.cmeis cmei3 ON c.cmei3_preferencia = cmei3.id
  LEFT JOIN public.cmeis cmei_remanejamento ON c.cmei_remanejamento_id = cmei_remanejamento.id
  LEFT JOIN public.turmas turma_atual ON c.turma_atual_id = turma_atual.id
...
GRANT EXECUTE ON FUNCTION public.get_fila_publica() TO anon, authenticated, service_role;
