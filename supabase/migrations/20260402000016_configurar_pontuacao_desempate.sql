...
ADD COLUMN IF NOT EXISTS peso_remanejamento integer NOT NULL DEFAULT 100;
...
BEGIN
  ...
  prioridade_remanejamento AS (
    SELECT 
      c.id AS crianca_id,
      CASE 
        WHEN cfg.prioridade_remanejamento_habilitada AND c.prioridade = 'Remanejamento' THEN cfg.peso_remanejamento
        ELSE 0
    ...
    SELECT
      ...
      (
        ...
        + CASE
            ...
            THEN (GREATEST(0, DATE_PART('day', now() - c.created_at))::int) * cfg.peso_data_cadastro
            ELSE 0
          END
      )::int AS score_cmei1,
      (
