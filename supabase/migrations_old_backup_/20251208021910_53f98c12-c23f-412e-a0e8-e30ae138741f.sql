...
BEGIN
  ...
  -- Nova lógica de ordenação com grupos claros:
  -- Grupo 1 (maior prioridade): Remanejamento SEM penalidade
  -- Grupo 2: Prioridade social SEM penalidade  
  -- Grupo 3: Fila geral SEM penalidade
  -- Grupo 4 (menor prioridade): Quem tem penalidade (fim de fila) - ordenado por data_penalidade
  ...
  posicoes_novas AS (
    SELECT 
      ...
      ROW_NUMBER() OVER (
        ORDER BY 
          -- Primeiro critério: Penalizados SEMPRE vão por último (grupo 0)
          CASE 
            WHEN c.data_penalidade IS NOT NULL THEN 0
            ELSE 1
          END DESC,
          -- Segundo critério (só para não-penalizados): tipo de prioridade
          CASE 
            WHEN c.data_penalidade IS NOT NULL THEN 0  -- Penalizados não ganham bônus
            ...
            ELSE 100  -- Fila geral sem prioridade
          END DESC,
          -- Terceiro critério: Peso das prioridades customizadas
          CASE 
            WHEN c.data_penalidade IS NOT NULL THEN 0
            ELSE COALESCE(ps.peso_total, 0) 
          END DESC,
          -- Quarto critério: Data de ordenação
          -- Penalizados: ordena pela data_penalidade (quem recebeu antes, fica antes)
          -- Não-penalizados: ordena pela data de entrada na fila
          CASE 
