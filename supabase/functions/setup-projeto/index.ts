...
serve(async (req) => {
  ...
  try {
    const setupSecret = Deno.env.get("SETUP_PROJETO_SECRET")?.trim();
    if (!setupSecret) {
      return new Response(JSON.stringify({ success: false, message: "secret_not_configured" }), {
        status: 500,
    ...
    if (!providedSecret || providedSecret !== setupSecret) {
      return new Response(JSON.stringify({ success: false, message: "nao_autorizado" }), {
        status: 401,
        headers: { ...getCorsHeaders(req), "Content-Type": "application/json" },
      });
    }
    ...
    const templates = [
      { tipo: 'convocacao', titulo: 'Convocação para Matrícula', assunto_email: 'Convocação - {nome_crianca}', corpo_whatsapp: 'Olá {nome_responsavel}! {nome_crianca} foi convocado para matrícula no {nome_cmei}. Prazo: {prazo_resposta}', ordem: 1 },
      { tipo: 'lembrete', titulo: 'Lembrete de Prazo', assunto_email: 'Lembrete - {nome_crianca}', corpo_whatsapp: 'Lembrete: prazo de matrícula de {nome_crianca} vence em {prazo_resposta}', ordem: 2 },
      { tipo: 'matricula_confirmada', titulo: 'Matrícula Confirmada', assunto_email: 'Matrícula Confirmada - {nome_crianca}', corpo_whatsapp: 'Matrícula de {nome_crianca} confirmada no {nome_cmei}!', ordem: 3 },
    ];
