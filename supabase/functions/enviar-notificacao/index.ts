...
// Templates de WhatsApp padrão (fallback)
const getDefaultWhatsAppTemplate = (tipo: string, variaveis: any) => {
  const { crianca_nome, responsavel_nome, cmei_nome, turma_nome, data_limite, protocolo, motivo, cmei_destino } = variaveis;

  const templates: Record<string, string> = {
    convocacao: `🎉 *Convocação para Matrícula - VAGOU*\n\nOlá, *${responsavel_nome}*!\n\nTemos o prazer de informar que *${crianca_nome}* foi convocado(a) para matrícula.\n\n📍 *CMEI:* ${cmei_nome}\n📚 *Turma:* ${turma_nome}\n⚠️ *Prazo limite:* ${data_limite}\n\n*Protocolo:* ${protocolo || 'N/A'}\n\nCompareça ao CMEI indicado com os documentos necessários dentro do prazo.`,
    
    lembrete_prazo: `⏰ *LEMBRETE: Prazo de Matrícula*\n\nOlá, *${responsavel_nome}*!\n\nEste é um lembrete sobre a convocação de *${crianca_nome}*.\n\n⚠️ *Data limite:* ${data_limite}\n📍 *CMEI:* ${cmei_nome}\n\n*Protocolo:* ${protocolo || 'N/A'}\n\nPor favor, compareça ao CMEI para confirmar a matrícula antes do vencimento do prazo.`,
    
    documentacao_pendente: `📄 *Documentação Necessária - VAGOU*\n\nOlá, *${responsavel_nome}*!\n\n*${crianca_nome}* foi convocado(a) e agora precisa do envio dos documentos para prosseguir com a matrícula.\n\n🔗 Acesse o sistema para enviar os documentos obrigatórios.\n⚠️ *Prazo:* ${data_limite}\n\n*Protocolo:* ${protocolo || 'N/A'}`,

    documento_recusado: `⚠️ *Documento Recusado - VAGOU*\n\nOlá, *${responsavel_nome}*!\n\nUm documento enviado para *${crianca_nome}* foi recusado e precisa ser reenviado.\n\n❌ *Motivo:* ${motivo || 'Documento não atende aos requisitos.'}\n\n🔗 Acesse o sistema para verificar e enviar uma nova versão.\n\n*Protocolo:* ${protocolo || 'N/A'}`,

    documentos_aprovados: `✅ *Documentação Aprovada - VAGOU*\n\nOlá, *${responsavel_nome}*!\n\nTodos os documentos de *${crianca_nome}* foram aprovados!\n\n📍 *Próximo Passo:* Compareça ao CMEI para *assinar a matrícula* presencialmente.\n\n*CMEI:* ${cmei_nome}\n*Protocolo:* ${protocolo || 'N/A'}`,

    matricula_confirmada: `🎉 *Matrícula Confirmada com Sucesso!*\n\nOlá, *${responsavel_nome}*!\n\nA matrícula de *${crianca_nome}* foi concluída com sucesso!\n\n✅ *CMEI:* ${cmei_nome}\n✅ *Turma:* ${turma_nome}\n\n*Protocolo:* ${protocolo || 'N/A'}\n\nParabéns! Aguarde o contato do CMEI para orientações sobre o início das aulas.`,

    inscricao_realizada: `✅ *Inscrição Realizada com Sucesso!*\n\nOlá, *${responsavel_nome}*!\n\nA inscrição de *${crianca_nome}* foi concluída no sistema VAGOU.\n\n📝 *Protocolo:* ${protocolo || 'N/A'}\n\nAcompanhe sua posição na fila pelo sistema. Você será notificado quando houver uma vaga disponível.`,

    remanejamento_solicitado: `📋 *Solicitação de Remanejamento Recebida*\n\nOlá, *${responsavel_nome}*!\n\nRecebemos sua solicitação de remanejamento para *${crianca_nome}*.\n\n📍 *Destino:* ${cmei_destino || 'A definir'}\n\nA criança continua matriculada no CMEI atual enquanto aguarda a nova vaga.\n\n*Protocolo:* ${protocolo || 'N/A'}`,

    remanejamento_concluido: `🔄 *Remanejamento Concluído*\n\nOlá, *${responsavel_nome}*!\n\nO remanejamento de *${crianca_nome}* foi concluído!\n\n✅ *Novo CMEI:* ${cmei_nome}\n✅ *Turma:* ${turma_nome}\n\n*Protocolo:* ${protocolo || 'N/A'}\n\nA criança já pode frequentar o novo CMEI.`,
    
    fim_fila: `⚠️ *Aviso: Fim da Fila*\n\nOlá, *${responsavel_nome}*!\n\n*${crianca_nome}* foi movido(a) para o final da fila de espera.\n\n❌ *Motivo:* ${motivo || 'Prazo excedido.'}\n\n*Protocolo:* ${protocolo || 'N/A'}\n\nVocê continuará na fila e será chamado novamente quando houver vaga.`,
  };
  ...
  return templates[tipoMapeado] || `Olá, *${responsavel_nome}*! Você recebeu uma notificação sobre *${crianca_nome}*. Acesse o sistema para mais informações.\n\n*Protocolo:* ${protocolo || 'N/A'}`;
};
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(JSON.stringify({ sucesso: false, erro: "nao_autorizado" }), {
        status: 401,
    ...
    userId = user?.id || null;
    if (authError || !userId) {
      return new Response(JSON.stringify({ sucesso: false, erro: "token_invalido" }), {
        status: 401,
        headers: { ...getCorsHeaders(req), "Content-Type": "application/json" },
      });
    }
    ...
    if (!crianca_id || !isUuid(crianca_id)) {
      return new Response(JSON.stringify({ sucesso: false, erro: "crianca_id_invalido" }), {
        status: 400,
        headers: { ...getCorsHeaders(req), "Content-Type": "application/json" },
      });
    }

    if (!tipo || typeof tipo !== "string" || tipo.trim().length > 80) {
      return new Response(JSON.stringify({ sucesso: false, erro: "tipo_invalido" }), {
        status: 400,
        headers: { ...getCorsHeaders(req), "Content-Type": "application/json" },
      });
    }

    const isResend = !!(dados_adicionais && (dados_adicionais as any).reenvio);

    if (tipo === "documentos_aprovados" && !isResend) {
      const { data: jaEnviado, error: jaEnviadoError } = await supabaseClient
      ...
      if (jaEnviadoError) {
        console.error("[Notificação] Erro ao verificar idempotência:", jaEnviadoError);
      } else if (jaEnviado && jaEnviado.length > 0) {
        return new Response(
          JSON.stringify({
            sucesso: true,
            skipped: true,
            motivo: "documentos_aprovados_ja_enviado",
          }),
          {
            headers: { ...getCorsHeaders(req), "Content-Type": "application/json" },
            status: 200,
    ...
    const responsavelUserId = (crianca as any)?.responsavel_user_id as string | null | undefined;
    if (!isAdmin && (!responsavelUserId || responsavelUserId !== userId)) {
      return new Response(JSON.stringify({ sucesso: false, erro: "sem_permissao" }), {
        status: 403,
        headers: { ...getCorsHeaders(req), "Content-Type": "application/json" },
      });
    }
    ...
    const variaveis: Record<string, string | number | null | undefined> = {
      ...
      protocolo: crianca.protocolo || null,
    ...
    const webhookPayload = {
      ...variaveis, // Espalha variáveis no topo do payload para facilitar no Make/Integromat
      tipo,
    ...
    // Seleção do telefone de destino para WhatsApp/SMS.
    // Principal = responsavel_telefone ("Telefone celular/WhatsApp")
    // Alternativo = responsavel_celular ("Telefone de contato 2")
    const usarContatoAlternativo = !!(dados_adicionais && (dados_adicionais as any).usar_contato_alternativo);
    const telefonePrincipal = crianca.responsavel_telefone || null;
    const telefoneAlternativo = crianca.responsavel_celular || null;
    const telefoneDestino = usarContatoAlternativo
      ? (telefoneAlternativo || telefonePrincipal)
      : (telefonePrincipal || telefoneAlternativo);

    const canalPreferido = ((crianca as any).canal_notificacao_preferido as string | null) || null;
    const canaisParaEnviar = canalPreferido ? [canalPreferido] : ['email', 'whatsapp', 'sms'];
    ...
    const prepararEmail = () => {
      if (template && template.corpo_email && template.assunto_email) {
        ...
        return {
          ...
          template_usado: 'banco_de_dados' as const,
        };
      }

      const fallback = getDefaultEmailTemplate(tipo, {
      ...
      return {
        ...
        html: fallback.html,
        template_usado: 'padrao' as const,
      };
    };

    const mensagemWhatsApp = template?.corpo_whatsapp
      ? processarTemplate(template.corpo_whatsapp, variaveis)
      : getDefaultWhatsAppTemplate(tipo, variaveis);

    const mensagemSMS = template?.corpo_sms
      ? processarTemplate(template.corpo_sms, variaveis)
      : mensagemWhatsApp;

    // Enviar notificação via Email (SMTP/Resend)
    if (canaisParaEnviar.includes('email') && config.notificacao_email && crianca.responsavel_email) {
      try {
        const emailData = prepararEmail();

        const emailWebhookUrl = (config as any).webhook_url_notificacao_email as string | null | undefined;
        const emailWebhookHabilitado = !!(emailWebhookUrl && isSafeWebhookUrl(emailWebhookUrl));

        if (emailWebhookHabilitado) {
          const payloadEmail = {
            ...webhookPayload,
            canal: 'email',
            assunto_email: emailData.subject,
            mensagem_email_html: emailData.html,
            mensagem_email: emailData.html,
            message: emailData.html,
            to_email: crianca.responsavel_email,
            to_name: crianca.responsavel_nome,
            is_resend: !!(dados_adicionais && (dados_adicionais as any).reenvio),
          };

          const webhookResponse = await fetch(emailWebhookUrl!, {
            method: 'POST',
          ...
          results.push({
            ...
            template_usado: emailData.template_usado,
            via: 'webhook',
          });
        } else {
      ...
      if (canaisParaEnviar.includes('email')) {
    ...
    const whatsappWebhookUrl = (config as any).webhook_url_notificacao as string | null | undefined;
    if (canaisParaEnviar.includes('whatsapp') && config.notificacao_whatsapp && whatsappWebhookUrl && isSafeWebhookUrl(whatsappWebhookUrl)) {
      try {
        console.log('[Notificação] Enviando WhatsApp via webhook');
        
        const webhookPayloadComMensagem = {
          ...webhookPayload,
          mensagem_whatsapp: mensagemWhatsApp || null,
          message: mensagemWhatsApp || null,
          to_phone: telefoneDestino,
          phone: telefoneDestino,
          to_email: crianca.responsavel_email || null,
          to_name: crianca.responsavel_nome,
          contato_alternativo: usarContatoAlternativo,
          is_resend: !!(dados_adicionais && (dados_adicionais as any).reenvio),
        };
    ...
    const smsWebhookUrl = (config as any).webhook_url_notificacao_sms as string | null | undefined;
    const smsWebhookHabilitado = !!(smsWebhookUrl && isSafeWebhookUrl(smsWebhookUrl));

    if (canaisParaEnviar.includes('sms') && config.notificacao_sms && telefoneDestino) {
      if (!smsWebhookHabilitado) {
        await supabaseClient.from('notificacoes_log').insert({
          crianca_id,
          tipo,
          canal: 'sms',
          ...
          erro: 'Webhook SMS não configurado',
          usuario_id: userId,
        });
        ...
        results.push({
          canal: 'sms',
          sucesso: false,
          mensagem: 'Webhook SMS não configurado',
        });
      } else {
        const payloadSMS = {
          ...webhookPayload,
          canal: 'sms',
          mensagem_sms: mensagemSMS || null,
          message: mensagemSMS || null,
          to_phone: telefoneDestino,
          phone: telefoneDestino,
          to_name: crianca.responsavel_nome,
          contato_alternativo: usarContatoAlternativo,
        ...
        const webhookResponse = await fetch(smsWebhookUrl!, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payloadSMS),
        });
        ...
        results.push({
          ...
          sucesso: webhookResponse.ok,
          status: webhookResponse.status,
          template_usado: template?.corpo_sms ? 'banco_de_dados' : 'fallback_whatsapp',
        });
      }
    } else {
      if (canaisParaEnviar.includes('sms')) {
        if (!config.notificacao_sms) console.log('[Notificação] Envio de SMS desativado nas configurações');
        if (!crianca.responsavel_telefone) console.log('[Notificação] Responsável sem telefone cadastrado');
      }
    }
