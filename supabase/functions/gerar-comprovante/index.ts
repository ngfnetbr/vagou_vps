...
serve(async (req: Request) => {
  ...
  try {
    ...
    // The comprovante can be generated by:
    //  - Admins (from admin panel)
    //  - Logged-in responsáveis (from responsavel dashboard)
    //  - Anonymous visitors right after submitting a public inscription
    // The anonymous case sends only the publishable/anon key as bearer, which
    // is a JWT WITHOUT a `sub` claim — calling `auth.getUser()` on it throws
    // "missing sub claim". So we try to identify the user, but never fail the
    // request when it's absent.
    ...
    if (authHeader) {
      try {
        ...
        console.log('[Comprovante] Chamada anônima (sem sessão de usuário)');
      }
    }
    ...
    const isResponsavel = user ? crianca.responsavel_user_id === user.id : false;
    // Anonymous callers (public inscription flow) are allowed — the comprovante
    // is the receipt for the inscription they just submitted.
    const isAnonymous = user === null;

    if (!isAdmin && !isResponsavel && !isAnonymous) {
      console.error(`[Comprovante] Acesso negado: Usuário ${user?.id} sem permissão para criança ${crianca_id}`);
      return new Response(
    ...
    console.log(`[Comprovante] Gerando ${tipo} para criança: ${crianca_id} (isAdmin: ${isAdmin}, isResponsavel: ${isResponsavel}, isAnonymous: ${isAnonymous})`);
