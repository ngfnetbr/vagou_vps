-- Corrigir a lógica: penalidade perde prioridade mas ordena por data_penalidade
-- Novos cadastros VÊM DEPOIS de quem pediu fim de fila
...
BEGIN
  ...
  -- 2. Dentro de cada grupo, ordena por data apropriada
  -- 3. Penalizados ordenam por data_penalidade (novos cadastros vêm DEPOIS)
  WITH posicoes_novas AS (
    SELECT 
      id,
      ROW_NUMBER() OVER (
        ORDER BY 
          -- Grupo de prioridade (penalizados perdem prioridade e vão pro grupo 3)
          CASE 
            WHEN data_penalidade IS NOT NULL THEN 3
          ...
          CASE 
            WHEN data_penalidade IS NOT NULL THEN data_penalidade
            ELSE COALESCE(data_retorno_fila, created_at)
          END ASC
      )::int as nova_posicao
    FROM criancas
    WHERE status IN ('Fila de Espera', 'Convocado')
