-- Atualizar função recalcular_posicoes_fila
-- Remover lógica de penalidade permanente (data_penalidade)
-- Agora usa apenas data_retorno_fila para ordenar quem voltou para a fila
...
BEGIN
  ...
  -- 1. Remanejamento (se habilitado)
  -- 2. Prioridade social (se habilitado)
  -- 3. Fila geral
  -- Dentro de cada grupo, ordena por data (data_retorno_fila se existir, senão created_at)
  -- Não há mais penalidade permanente - fim de fila é apenas uma redefinição de data
  ...
  posicoes_novas AS (
    SELECT 
      ...
      ROW_NUMBER() OVER (
        ORDER BY 
          -- Primeiro critério: tipo de prioridade
          CASE 
          ...
          -- Se tem data_retorno_fila, usa ela (voltou à fila depois)
          -- Senão usa created_at (cadastro original)
