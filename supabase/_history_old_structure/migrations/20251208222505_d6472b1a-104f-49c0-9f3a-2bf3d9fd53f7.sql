...
AS $$
  SELECT 
    c.id,
    -- Nome da criança abreviado (primeiro nome + inicial do sobrenome)
    CASE 
    ...
    -- Nome do responsável abreviado (primeiro nome + inicial do sobrenome)
    CASE 
      WHEN position(' ' in c.responsavel_nome) > 0 
      THEN split_part(c.responsavel_nome, ' ', 1) || ' ' || substr(split_part(c.responsavel_nome, ' ', array_length(string_to_array(c.responsavel_nome, ' '), 1)), 1, 1) || '.'
