...
const processarTemplate = (
  ...
  Object.entries(variaveis).forEach(([chave, valor]) => {
    // Suporta {{chave}}, {chave}, {{ chave }} e { chave } (insensível a maiúsculas/minúsculas)
    // Escapa as chaves para garantir que sejam tratadas como literais no regex
    const regexDouble = new RegExp(`\\{\\{\\s*${chave}\\s*\\}\\}`, "gi");
    const regexSingle = new RegExp(`\\{\\s*${chave}\\s*\\}`, "gi");
    
    const valorString = String(valor ?? "");
    resultado = resultado.replace(regexDouble, valorString);
    resultado = resultado.replace(regexSingle, valorString);
  });
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const variaveis: Record<string, string | number | null | undefined> = {
      ...
      protocolo: dados_adicionais?.protocolo || crianca.protocolo || null,
