...
const gerarCabecalhoPDF = (config: any, titulo: string) => {
  const brasaoHtml = config?.brasao_url 
    ? `<div style="display: flex; justify-content: center; margin-bottom: 8px;">
        <img src="${config.brasao_url}" alt="Brasão" style="height: 50px; width: auto; display: block; margin: 0 auto;" />
       </div>`
    : '';

  const contatoInfo = [];
  if (config?.email_contato) contatoInfo.push(`E-mail: ${config.email_contato}`);
  if (config?.telefone_contato) contatoInfo.push(`Tel: ${config.telefone_contato}`);
...
serve(async (req) => {
  ...
  try {
    ...
    const cmeiIds = [...new Set([
      ...((historico || []).map(h => h.cmei_anterior).filter(Boolean)),
      ...((historico || []).map(h => h.cmei_novo).filter(Boolean))
    ])];
    const turmaIds = [...new Set([
      ...((historico || []).map(h => h.turma_anterior).filter(Boolean)),
      ...((historico || []).map(h => h.turma_novo).filter(Boolean))
    ])];
    ...
    const cmeiMap = Object.fromEntries((cmeisData || []).map(c => [c.id, c.nome]));
    const turmaMap = Object.fromEntries((turmasData || []).map(t => [t.id, t.nome]));
...
<body>
  ...
  <div class="protocolo">
    <strong>Protocolo:</strong> ${crianca.id.substring(0, 8).toUpperCase()}
  </div>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      <div class="info-row">
        ...
        <div class="info-value">${crianca.nome}</div>
      </div>
      <div class="info-row">
        <div class="info-label">Data de Nascimento:</div>
        <div class="info-value">${new Date(crianca.data_nascimento).toLocaleDateString('pt-BR')}</div>
      </div>
      <div class="info-row">
        <div class="info-label">Sexo:</div>
        <div class="info-value">${crianca.sexo}</div>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      <div class="info-row">
        <div class="info-label">Nome:</div>
        <div class="info-value">${crianca.responsavel_nome}</div>
      </div>
      <div class="info-row">
        <div class="info-label">CPF:</div>
        <div class="info-value">${crianca.responsavel_cpf}</div>
      </div>
      <div class="info-row">
        <div class="info-label">Telefone:</div>
        <div class="info-value">${crianca.responsavel_telefone}</div>
      </div>
      ${crianca.responsavel_email ? `
      <div class="info-row">
        <div class="info-label">E-mail:</div>
        <div class="info-value">${crianca.responsavel_email}</div>
      </div>
      ` : ''}
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      <div class="info-row">
        ...
        <div class="info-value">${crianca.status}</div>
      </div>
      ${crianca.posicao_fila ? `
      <div class="info-row">
        <div class="info-label">Posição na Fila:</div>
        <div class="info-value">${crianca.posicao_fila}ª posição</div>
      </div>
      ` : ''}
      <div class="info-row">
        <div class="info-label">Data da Inscrição:</div>
        <div class="info-value">${new Date(crianca.created_at).toLocaleDateString('pt-BR')}</div>
      </div>
      ${crianca.cmei1?.nome ? `
      <div class="info-row">
        <div class="info-label">1ª Preferência CMEI:</div>
        <div class="info-value">${crianca.cmei1.nome}</div>
      </div>
      ` : ''}
      ${crianca.cmei2?.nome ? `
      <div class="info-row">
        <div class="info-label">2ª Preferência CMEI:</div>
        <div class="info-value">${crianca.cmei2.nome}</div>
      </div>
      ` : ''}
    </div>
  </div>
...
<body>
  ...
  <div class="alert">
    ...
    <p>Você tem até <strong>${crianca.convocacao_deadline ? new Date(crianca.convocacao_deadline).toLocaleDateString('pt-BR') : 'a definir'}</strong> para comparecer ao CMEI e realizar a matrícula.</p>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      <div class="info-row">
        ...
        <div class="info-value">${crianca.cmei_atual?.nome || 'A definir'}</div>
      </div>
      ${crianca.turma_atual?.nome ? `
      <div class="info-row">
        <div class="info-label">Turma:</div>
        <div class="info-value">${crianca.turma_atual.nome}</div>
      </div>
      ` : ''}
      ${crianca.turma_atual?.turno ? `
      <div class="info-row">
        <div class="info-label">Turno:</div>
        <div class="info-value">${crianca.turma_atual.turno}</div>
      </div>
      ` : ''}
      ${crianca.cmei_atual?.endereco ? `
      <div class="info-row">
        <div class="info-label">Endereço:</div>
        <div class="info-value">${crianca.cmei_atual.endereco}</div>
      </div>
      ` : ''}
      ${crianca.cmei_atual?.telefone ? `
      <div class="info-row">
        <div class="info-label">Telefone:</div>
        <div class="info-value">${crianca.cmei_atual.telefone}</div>
      </div>
      ` : ''}
    </div>
  </div>
...
<body>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      ...
      <div class="info-row">
        <div class="info-label">CPF:</div>
        <div class="info-value">${crianca.cpf_crianca}</div>
      </div>
      ` : ''}
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      <div class="info-row">
        <div class="info-label">CMEI:</div>
        <div class="info-value">${crianca.cmei_atual?.nome || 'Não informado'}</div>
      </div>
      <div class="info-row">
        <div class="info-label">Turma:</div>
        <div class="info-value">${crianca.turma_atual?.nome || 'Não informado'}</div>
      </div>
      ${crianca.turma_atual?.turno ? `
      <div class="info-row">
        <div class="info-label">Turno:</div>
