...
export default function CadastrosInstituicoesConsulta() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["sondagem-consulta-cmeis"],
    queryFn: async () => {
  ...
  const { data: occupationStats } = useQuery({
    queryKey: ["sondagem-consulta-cmeis-occupation"],
    queryFn: async () => {
      ...
      ;((data || []) as Array<{ cmei_id: string | null; capacidade: number | null; criancas?: Array<{ count: number }> | null }>).forEach((t) => {
        const id = t.cmei_id
        if (!id) return
